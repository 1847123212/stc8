...
typedef struct {
    INT8U   OSEventType;//时间类型                   /* Type of event control block (see OS_EVENT_TYPE_???)      */
    INT8U   OSEventGrp; //等待任务所在的组                   /* Group corresponding to tasks waiting for event to occur  */
    INT16U  OSEventCnt; //计数器                   /* Semaphore Count (not used if other EVENT type)           */
    void   *OSEventPtr; //指向消息或消息列队的指针                   /* Pointer to message or queue structure                    */
    INT8U   OSEventTbl[OS_EVENT_TBL_SIZE];//等待任务列表 /* List of tasks waiting for event to occur                 */
} OS_EVENT;
#endif
...
OS_EXT idata INT32U       OSCtxSwCtr;               /* Counter of number of context switches           */
...
OS_EXT idata INT8U        OSIntNesting;             /* Interrupt nesting level                         */
OS_EXT idata INT8U        OSIntExitY;

OS_EXT idata INT8U        OSLockNesting;            /* Multitasking lock nesting level                 */

OS_EXT idata INT8U        OSPrioCur;                /* Priority of current task                        */
OS_EXT idata INT8U        OSPrioHighRdy;            /* Priority of highest priority task               */

OS_EXT idata INT8U        OSRdyGrp;                        /* Ready list group                         */
OS_EXT  INT8U             OSRdyTbl[OS_RDY_TBL_SIZE];       /* Table of tasks which are ready to run    */

OS_EXT idata BOOLEAN      OSRunning;                       /* Flag indicating that kernel is running   */

OS_EXT idata INT8U        OSTaskCtr;                       /* Number of tasks created                  */

OS_EXT idata INT32U       OSIdleCtr;                                 /* Idle counter                   */
...
OS_EXT  idata OS_TCB           *OSTCBFreeList;                   /* Pointer to list of free TCBs             */
OS_EXT  idata OS_TCB     *OSTCBHighRdy;                 /* Pointer to highest priority TCB R-to-R   */
OS_EXT  idata OS_TCB           *OSTCBList;                       /* Pointer to doubly linked list of TCBs    */
OS_EXT  idata OS_TCB           *OSTCBPrioTbl[OS_LOWEST_PRIO + 1];/* Table of pointers to created TCBs        */
